...
Three Petri Net machines in a single multi-rate runtime (one of each prior
example):
...
`PnRuntime`.  Each net registers its own `period_us`:

```
rt.add_net(light_a,   10_000)   # 10 ms
rt.add_net(blink_b,   10_000)   # 10 ms
rt.add_net(auto_c,    20_000)   # 20 ms
...
- PN modules reused from `examples/pn/01-light/`, `02-auto/`, `03-blink/`
...
uv run examples/pn/04-mix/main.py           # cyclic executive

uv run examples/pn/04-mix/main_coop.py      # cooperative deadline scheduler

uv run examples/pn/04-mix/main_threads.py   # BSP thread-per-node

uv run examples/pn/04-mix/main_async.py     # asyncio (legacy)
```
