...
/*
 * PN 04-mix — cooperative single-thread scheduler + tracing.
 *
 * Identical to main_coop.c but with the rxnet tracing subsystem enabled.
 * Single-threaded: the tracer mutex is never contested, overhead is minimal.
 *
 * Extra CLI command:
 *   trace          — export trace.bin and print visualisation instructions
 *
 * Build:
 *   make -C c pn_04_mix_coop_trace
 *   ./c/build/pn_04_mix_coop_trace
 *   > trace
