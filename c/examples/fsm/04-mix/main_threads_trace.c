...
/*
 * FSM 04-mix — parallel thread-per-node scheduler + tracing.
 *
 * Identical to main_threads.c but with the rxnet tracing subsystem enabled.
 * The tracer mutex is thread-safe: multiple node threads write concurrently
 * and the CLI thread exports without data races.
 *
 * Extra CLI command:
 *   trace          — export trace.bin and print visualisation instructions
 *
 * Build:
 *   make -C c fsm_04_mix_threads_trace
 *   ./c/build/fsm_04_mix_threads_trace
 *   > trace
