...
static unsigned int blink_effective_hz(const rx_fsm_machine *machine) {
...
cmd_status(rx_fsm_context *ctx, cli_machine_data *cli,
           const char *command, void *command_user_data)
{
    const mix_cli_app_data *app = (const mix_cli_app_data *)command_user_data;
...
cmd_freq(rx_fsm_context *ctx, cli_machine_data *cli,
         const char *command, void *command_user_data)
{
    mix_cli_app_data *app = (mix_cli_app_data *)command_user_data;
    ...
    if (blink_fsm_set_base_hz(app->blink_b_machine, freq_hz) != 0) {
            ...
            const char *command, void *command_user_data)
{
    ...
    if (auto_fsm_set_auto_off_timeout_ms(app->auto_c_machine, timeout_ms) != 0) {
...
main(void)
{
    ...
    if (rx_fsm_runtime_add_machine(&runtime, &light_a_machine) != 0 ||
        rx_fsm_runtime_add_machine(&runtime, &blink_b_machine) != 0 ||
